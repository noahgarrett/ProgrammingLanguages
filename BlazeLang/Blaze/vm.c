...
void initVM() {
	...
	// Nulling the initString just in case the copyString method triggers the garbage collector before it can be initialized
	vm.initString = NULL;
	...
	if (IS_OBJ(callee)) {
		switch (OBJ_TYPE(callee)) {
			...
			case OBJ_CLASS: {
				...
				// Automatically call the init method on new instances
				Value initializer;
				if (tableGet(&klass->methods, vm.initString, &initializer)) {
					return call(AS_CLOSURE(initializer), argCount);
				}
				else if (argCount != 0) {
					runtimeError("Expected 0 argyuments but got %d.", argCount);
					return false;
				}
