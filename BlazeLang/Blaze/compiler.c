...
Compiler* current = NULL; // Once going multi-threaded capabilities, this needs to stop being a global variable
...
static void addLocal(Token name) {
	if (current->localCount == UINT8_COUNT) {
		error("Too many local variables in function. (Limit = 256)");
		return;
	}
...
static void declareVariable() {
	...
	for (int i = current->localCount - 1; i >= 0; i--) {
		...
		if (identifiersEqual(name, &local->name)) {
			error("Already a variable with the same name defined in this scope.");
		}
	}
...
static void super_(bool canAssign) {
	...
	// TODO: Figure out how to implement passing variables into a superclass' init method | Ex. Python: super(val1, val2)
...
static void forStatement() {
	...
	// Incrementing a value after end of loop
